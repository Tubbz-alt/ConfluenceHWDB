...
class EFUSEField(page.Field):
    def __init__(self, efuse):
        super(EFUSEField, self).__init__(key="EFUSE", value=efuse)
        
    def isValid(self):
        if self.getValue() == "":
            print "EFUSE must not be empty string"
            return False
        return True    
...
def buildEFUSEFields():
    while True:
        ans = raw_input("Enter EFUSE value: ").strip()
        if ans == ".":
            raise AbortPage
        if ans == "":
            return None
        try:
            return EFUSEField(ans)
        except ValueError:
            pass            
